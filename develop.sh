...
if [ $# -gt 0 ];then
    case "$1" in
        ...
        "score-compose")
            shift
            docker run --rm -it -v $(pwd):/score-compose ghcr.io/score-spec/score-compose:latest "$@"
            ;;
        "score-k8s")
            shift
            docker run --rm -it -v $(pwd):/score-k8s ghcr.io/score-spec/score-k8s:latest "$@"
            ;;
        "phpcs")
            shift
        ...
        "rector")
            shift
            docker run --rm -itv $(pwd):/app -w /app php:8.2.18-fpm php vendor/bin/rector process src
            ;;
        "security-checker")
            shift
        ...
        "integration")
            ...
            ;;
        "analyze")
            docker run --rm -itv $(pwd):/app -w /app php:8.2.18-fpm php vendor/bin/phpcs --standard=phpcs.xml --extensions=php --report=full src
            ;;
        "pre-commit-phpcs")
            php vendor/bin/phpcs --standard=phpcs.xml src
           ;;
        "pre-commit-static-analysis-changed")
            $(git --no-pager diff --name-only --diff-filter=MARC| grep -E 'src/')
            if [ $? -eq 0 ]; then
                docker run --rm -v /$(pwd):/app phpstan/phpstan analyse --level max --no-progress $(git --no-pager diff --name-only --diff-filter=MARC| grep -E 'src/')
            fi
           ;;
#        "jsinstall")
